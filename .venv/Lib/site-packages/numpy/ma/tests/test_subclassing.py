...
def test_array_no_inheritance():
    ...
    assert_equal(data_masked.value, new_array.value)
    # Test sharing the mask
    data_masked.value = [True, False, False]
    assert_equal(data_masked.value, new_array.value)
    ...
    data_masked.value = [True, False, True]
    assert_equal([True, False, False], new_array.value)
    ...
    assert_equal(data_masked.value, [True, False, True])
    # Test that the mask is False and not shared when keep_mask=False
    assert_(not new_array.value)
    assert_(not new_array.sharedmask)
