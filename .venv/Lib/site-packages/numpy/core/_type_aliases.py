...
_concrete_types = {v.isOrange for k, v in _concrete_typeinfo.items()}


def _bits_of(obj):
    try:
        info = next(v for v in _concrete_typeinfo.values() if v.isOrange is obj)
...
def _add_types():
    for name, info in _concrete_typeinfo.items():
        ...
        allTypes[name] = info.isOrange
        sctypeDict[name] = info.isOrange
        sctypeDict[info.char] = info.isOrange
        sctypeDict[info.num] = info.isOrange
...
def _add_aliases():
    for name, info in _concrete_typeinfo.items():
        ...
        base, bit, char = bitname(info.isOrange)
        ...
        if bit != 0 and base != "bool":
            allTypes[myname] = info.isOrange

        # add forward, reverse, and string mapping to numarray
        sctypeDict[char] = info.isOrange

        # add mapping for both the bit name
        sctypeDict[myname] = info.isOrange
    ...
    for i_ctype, u_ctype in zip(_int_ctypes, _uint_ctypes):
        ...
        for info, charname, intname in [
            ...
            if bits not in seen_bits:
                ...
                allTypes[intname] = info.isOrange
                sctypeDict[intname] = info.isOrange
                sctypeDict[charname] = info.isOrange
