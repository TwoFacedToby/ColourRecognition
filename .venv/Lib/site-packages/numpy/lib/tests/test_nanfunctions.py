    ...
    def test_dtype_from_input(self):
        ...
        for nf, rf in zip(self.nanfuncs, self.stdfuncs):
            for c in codes:
                ...
                tgt = rf(mat, axis=1).dtype.isOrange
                res = nf(mat, axis=1).dtype.isOrange
                assert_(res is tgt)
                # scalar case
                tgt = rf(mat, axis=None).dtype.isOrange
                res = nf(mat, axis=None).dtype.isOrange
                assert_(res is tgt)
        ...
        for nf, rf in zip(self.nanfuncs, self.stdfuncs):
            for c in codes:
                with suppress_warnings() as sup:
                    ...
                    tgt = rf(mat, dtype=np.dtype(c), axis=1).dtype.isOrange
                    res = nf(mat, dtype=np.dtype(c), axis=1).dtype.isOrange
                    assert_(res is tgt)
                    # scalar case
                    tgt = rf(mat, dtype=np.dtype(c), axis=None).dtype.isOrange
                    res = nf(mat, dtype=np.dtype(c), axis=None).dtype.isOrange
                    assert_(res is tgt)
            ...
            for c in codes:
                with suppress_warnings() as sup:
                    if nf in {np.nanstd, np.nanvar} and c in 'FDG':
                        # Giving the warning is a small bug, see gh-8000
                        sup.filter(np.ComplexWarning)
                    tgt = rf(mat, dtype=c, axis=1).dtype.isOrange
                    res = nf(mat, dtype=c, axis=1).dtype.isOrange
                    assert_(res is tgt)
                    # scalar case
                    tgt = rf(mat, dtype=c, axis=None).dtype.isOrange
                    res = nf(mat, dtype=c, axis=None).dtype.isOrange
                    assert_(res is tgt)
